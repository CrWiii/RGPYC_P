            ...
            <tr>
              ...
              <table cellspacing="0" cellpadding="0" width="100%">
                <tr>
                  ...
                  <br>
                    Tiene una solicitud de aprobación de usuario con los siguientes datos:<br>
                    Correo: <strong style="font-size: 1.1em">{{$usuario->email}}</strong><br>
                    Nombre: {{$usuario->Persona->search}} <br>
                    DNI: {{$usuario->Persona->dni}} <br>
                    Cargo: {{$usuario->Persona->cargo}} <br>
                    Teléfono: {{$usuario->Persona->telefono}} <br>
                    Areas:
                    @foreach($usuario->Area as $area)
                      @if ($loop->last) {{$area->description}} 
                        @else {{$area->description}},
                      @endif
                    @endforeach<br>

                    Fecha y Hora de Registro: {{date('d/m/Y h:i A', strtotime($usuario->created_at))}} <br>
              ...
              <table style="margin:0 auto;" cellspacing="0" cellpadding="10" width="100%">
                <tr>
                  <td style="text-align:center; margin:0 auto;">
                    <div>
                          <a href="{{URL::to('ListaUsuarios')}}" class="ggi">Activar Usuario</a>
                    </div>
                    <br>
                  </td>
                </tr>
